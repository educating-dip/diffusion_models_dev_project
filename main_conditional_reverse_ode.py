...
from src import (marginal_prob_std, diffusion_coeff, OpenAiUNetModel, EllipseDatasetFromDival, simple_trafo, get_walnut_2d_ray_trafo, get_walnut_data_on_device,
   	simulate, PSNR, SSIM, get_disk_dist_ellipses_dataset, limited_angle_trafo, ode_sampler)
...
parser.add_argument("--use_new_forward", action='store_true')
...
def coordinator(args):
	...
	if args.use_new_forward:
		save_root = Path(f'./results/{args.dataset}/sampler=ode/start_N={config.sampling.start_time_step}/penalty={args.penalty}/angular_range={args.angular_range}')
	else:
		save_root = Path(f'./results/{args.dataset}/sampler=ode/start_N={config.sampling.start_time_step}/penalty={args.penalty}')
