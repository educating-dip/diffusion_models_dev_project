...
from src import (cg, PSNR, SSIM, SingleCoilMRI, get_standard_sde, get_standard_score, get_standard_sampler, get_standard_adapted_sampler) 
...
parser.add_argument('--cg_iter', default=1)
...
parser.add_argument('--num_optim_step', default=3, help='num. of optimization steps taken per sampl. step')
...
#img_file = "/localdata/AlexanderDenker/MRI/Dataset/Brain/label/file_brain_AXFLAIR_200_6002447/007.png" 

img_file = "/localdata/AlexanderDenker/MRI/Dataset/Knee/label/file1000041/025.png"
...
logg_kwargs = {'log_dir': ".", 'num_img_in_log': 5,
		'sample_num':0, 'ground_truth': torch.abs(ground_truth).cpu(), 'filtbackproj': torch.abs(filtbackproj).cpu()}
...
recon_dds = sampler.sample(logg_kwargs=logg_kwargs, logging=True)
