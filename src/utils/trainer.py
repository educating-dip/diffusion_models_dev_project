...
def cond_score_model_trainer(
	...
	for epoch in range(optim_kwargs['epochs']):
		...
		for idx, batch in tqdm(enumerate(train_dl), total = len(train_dl)):
			...
			x = x.to(device)
			y = y.to(device)
			loss = cond_loss_fn(score, x, y, sde)
			optimizer.zero_grad()
			loss.backward()    
			optimizer.step()
...
def simple_cond_sampler(score, sde, img_shape, y, num_steps, snr, eps):
	...
	with torch.no_grad():
		for time_step in tqdm(time_steps):
			...
			grad = score(x, y, batch_time_step)
			...
			x_mean = x - (f - (g**2)[:, None, None, None] * score(x, y, batch_time_step)) * step_size
